...
// File and directory deletion calls in lowercased Python, Node.js, Perl,
// Ruby and PHP one-liners.
const INLINE_CODE_DELETION = new RegExp([
  String.raw`\b(?:shutil\.rmtree|os\.(?:remove|unlink|rmdir|removedirs)|send2trash`,
  String.raw`|rmsync|rmdirsync|unlinksync|rimraf|fileutils\.(?:rm|remove)\w*`,
  String.raw`|file\.(?:delete|unlink)|dir\.(?:delete|rmdir|unlink)|rmtree|remove_tree|unlink)\b`,
  String.raw`|\b(?:rm|rmdir)\s*\(`
].join(''));
// PowerShell reaching .NET deletion without a deletion command name:
// [IO.File]::Delete(...), (Get-Item x).Delete() and $_.Delete().
const DOTNET_DELETION = /\[(?:system\.)?io\.(?:file|directory|fileinfo|directoryinfo)\]::delete\s*\(|(?:\)|\$[\w:]+)\.delete\s*\(/;
...
// Flags that hand a one-line program to an interpreter (python -c, node -e).
function inlineCodeFlags(name) {
  if (/^(?:python\d*(?:\.\d+)*|py)$/.test(name)) return ['-c'];
  if (name === 'node') return ['-e', '--eval', '-p', '--print'];
  if (name === 'perl' || name === 'ruby') return ['-e'];
...
function classifyDelegatedShellCommand(command) {
  ...
  if (includesAny(entries.map(entry => entry.name), destructiveFiles)
      ...
      || DOTNET_DELETION.test(String(command || '').toLowerCase())) {
  ...
  for (const entry of entries) {
    ...
    const inlineFlags = inlineCodeFlags(entry.name);
    const codeIndex = inlineFlags ? args.findIndex(arg => inlineFlags.includes(arg)) : -1;
    if (codeIndex >= 0 && INLINE_CODE_DELETION.test(args.slice(codeIndex + 1).join(' '))) {
      return highRisk('file_delete', '命令会删除文件、目录或系统属性。');
    }
  }
